...
export const sendRegistrationUrl = async (email: string, hash: string, name: string): Promise<void> => {
  const link = `${configVars.FRONTEND_URL}/register?hash=${hash}`;
  const userName = name;

  await transporter.sendMail({
    from: `Olga Finance <${configVars.SMTP_USER}>`,
    to: email,
